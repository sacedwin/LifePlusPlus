...
public class EstablishmentDB
{
    ...
    public static List<Establishment> getAllEstablishments()
    {
        ...
        try
        {
            ...
            while (reader.Read())
            {
				...
				e.TempEstID = reader["tempEstablishmentID"].ToString();
	...
	public static int updateestStatus(Establishment u)
	{
		int result;
		try
		{
			SqlCommand command = new SqlCommand("Update establishment set status=@status,adminID=@adminID where establishmentID=@establishmentID");
			command.Parameters.AddWithValue("@establishmentID", u.ID);
			command.Parameters.AddWithValue("@adminID", u.Admin.adminId);
			command.Parameters.AddWithValue("@status", u.Status);
			command.Connection = connection;
			connection.Open();
			result = command.ExecuteNonQuery();
